            ...
            if (!in_array($audio[7], $storyArray)) {
                ...
                if (!in_array($audio[8], $storyArray[$audio[7]])) {
                    ...
                    if ($section != "" && $section == $audio[8]) {
                        ...
                        echo '<a href="javascript:void(0);" onclick="openDialog(\'' . $audio[4] . '\', \'' . utf8_decode(htmlentities(addslashes($audio[2]))) . '\',' . $key . ');">' . PHP_EOL;
                        echo '<img style="max-width: 250px; max-height: 250px;" src="Thumbnails/' . $img . '" alt="(image unavaliable)"/>' . PHP_EOL;
                        echo  utf8_decode(htmlentities($audio[2])) . '</a><br />' . PHP_EOL;
                        echo '</div>' . PHP_EOL;
